...
TEST_F(SPTest, SelfConnect) {
  ...
  EXPECT_EQ(grid.get_min_moves_norestrict(Coordinates{2, 2}, Coordinates{2, 2}), 0);
}

TEST_F(SPTest, OutOfBoundsConnect) {
  Grid grid(3, 3);
  EXPECT_EQ(grid.get_num_moves(Coordinates{2, 2}, Coordinates{2, 3}), std::numeric_limits<int>::max());
  EXPECT_THROW(grid.get_min_moves_norestrict(Coordinates{2, 2}, Coordinates{2, 3}), std::runtime_error);
}

TEST_F(SPTest, Connect_Horz_Pos_Left_1) {
  Grid grid(3, 3);
  EXPECT_EQ(grid.get_num_moves(Coordinates{2, 2}, Coordinates{1, 2}), 1);
  EXPECT_EQ(grid.get_min_moves_norestrict(Coordinates{2, 2}, Coordinates{1, 2}), 1);
}
...
TEST_F(SPTest, Connect_Horz_Pos_Right_1) {
  ...
  EXPECT_EQ(grid.get_min_moves_norestrict(Coordinates{2, 2}, Coordinates{2, 3}), 1);
}

TEST_F(SPTest, Connect_Horz_Pos_Right_2) {
  Grid grid(4, 4);
  EXPECT_EQ(grid.get_num_moves(Coordinates{3, 1}, Coordinates{3, 3}), 2);
  EXPECT_EQ(grid.get_min_moves_norestrict(Coordinates{3, 1}, Coordinates{3, 3}), 2);
}
...
TEST_F(SPTest, Connect_Multi_Points) {
  ...
  EXPECT_EQ(grid.get_min_moves_norestrict(Coordinates{1, 1}, targets.begin(), targets.end()), expected_moves);
}
